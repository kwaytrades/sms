    ...
    def _determine_engines(self, intent_analysis: Dict, user_context: Dict) -> List[Dict[str, Any]]:
        ...
        if primary_intent == "analyze":
            if symbols:
                ...
                # 🧠 SMART: Use conversation context to infer symbols
                ...
                # Use contextual symbols for analysis
                engines.append({
                    "engine": "technical_analysis",
                    "symbols": contextual_symbols,
                    "priority": "high"  # High priority since user is continuing conversation
                })
                
                # If they ask about fundamentals specifically, include fundamental analysis
                original_message = intent_analysis.get("original_message", user_context.get("current_message", "")).lower()
                ...
                # Add news if they're asking follow-up questions (shows continued interest)
        ...
        elif primary_intent in ["worry_check", "validation_seeking"] and (symbols or recent_symbols):
            # For emotional support, get current data + news
            target_symbols = symbols if symbols else recent_symbols[:2]
            engines.append({
                "engine": "technical_analysis",
        ...
        elif primary_intent == "celebrate" and (symbols or recent_symbols):
            # For celebration, just need current price confirmation
            target_symbols = symbols if symbols else recent_symbols[:1]
            engines.append({
                "engine": "technical_analysis",
                "symbols": target_symbols,
                "priority": "medium"
